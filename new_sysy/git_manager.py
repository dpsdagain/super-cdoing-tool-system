...
class GitDiffInput(BaseModel):
    file_path: Optional[str] = Field(
        None, description="Optional path to a specific file to diff."
    )
...
def git_diff(file_path: Optional[str] = None) -> str:
    ...
    try:
        ...
        logger.exception("Error running git diff:")
...
def git_commit(message: str) -> str:
    ...
    try:
        ...
        logger.exception("Error running git commit:")
        return f"Error running git commit: {str(e)}"


@register_tool(
    name="git_root",
    description="Get the absolute path to the root of the git repository.",
    input_schema=GitStatusInput,
    is_read_only=True,
)
def git_root() -> str:
...
def git_log(limit: int = 5) -> str:
    ...
    try:
        ...
        logger.exception("Error running git log:")
        return f"Git not found or error: {str(e)}"
