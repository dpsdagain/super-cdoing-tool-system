...
class WorkerAgent:
    ...
    def __init__(
        self, delegation_depth: int, main_model: str, permission_mode: str = "ASK"
    ):
...
class Coordinator:
    ...
    def delegate(self, task: str, context_summary: str = "") -> str:
        ...
        """
        if self.manager_engine.config.delegation_depth >= 3:
            return "Error: Maximum delegation depth reached."

        worker = WorkerAgent(
            delegation_depth=self.manager_engine.config.delegation_depth + 1,
