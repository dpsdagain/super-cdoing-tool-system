...
from search_engine import (
    _get_pinned_embedding, get_reranker, hybrid_search, 
    calculate_cosine_similarity, _sort_docs_deterministically, _rewrite_executor
)
...
def _prepare_history_with_cache(history: list[BaseMessage], model: str | None) -> list[BaseMessage]:
    ...
    if len(user_msg_indices) >= 2:
        ...
        if target_idx >= 0:
            ...
            new_msg = AIMessage(content=format_message_content(msg.content, model, use_cache=True))
...
class SpecialistRouter:
    ...
    def route(self, user_input: str, history: list, enable_auto: bool) -> tuple[str, str, str | None]:
        ...
        specialist_model = SPECIALIST_MAPPING.get(specialty) if specialty != "GENERAL" else None
        
        # Guard: Local model should not route to cloud specialist
        if (specialist_model and not specialist_model.startswith(OLLAMA_PREFIX) and 
            self.main_model and self.main_model.startswith(OLLAMA_PREFIX)):
    ...
    def get_active_llm(self, specialist_model: str | None, base_llm, prompt):
        ...
        if specialist_model:
            if specialist_model not in self._specialist_llm_cache:
                self._specialist_llm_cache[specialist_model] = get_llm(model=specialist_model, streaming=True)
...
class ContextBudgeter:
    ...
    def get_budget(self) -> int:
        _CONTEXT_BUDGETS = {
            "claude-3-5": 160000, "claude-3": 120000, "gpt-4": 80000,
            "gemini": 250000, "deepseek": 40000, "qwen": 24000,
        }
        _budget = 28000
    ...
    def process_union(self, new_docs: list[Document], previous_union: list[Document], intent: str) -> list[Document]:
...
class ContextCacheChain:
    ...
    def _on_sentinel_done(self, future):
        try:
            future.result(); self._sentinel_failures["count"] = 0
        except Exception as e:
            self._sentinel_failures["count"] += 1
            logger.error(f"Sentinel failure ({self._sentinel_failures['count']}): {e}")

    def _prepare_context_strings(self, final_docs, previous_union, intent):
        stable_hashes = {d.metadata.get("content_hash", "") for d in previous_union} if previous_union and intent == "FOLLOW-UP" else None
        established_docs, new_docs = [], []
        if stable_hashes:
            for d in final_docs:
                if d.metadata.get("content_hash", "") in stable_hashes: established_docs.append(d)
                else: new_docs.append(d)
        else: new_docs = final_docs
    ...
    def __call__(self, inputs: dict):
        ...
        if not inputs.get("force_retrieval", False):
            cached = sem_cache.lookup(user_input, threshold=SEMANTIC_CACHE_THRESHOLD, collection_scope=coll_name, pinned_content=pinned_content, model=self.model)
            if cached: yield {"answer": cached, "intent": "CACHE_HIT"}; return

        # 2. Intent & Specialist Routing
        enable_auto = inputs.get("auto_specialist", ENABLE_AUTO_SPECIALIST)
        intent, specialty, specialist_model = self.specialist_router.route(user_input, history, enable_auto)
        ...
        new_retrievals = hybrid_search(
            self.db, user_input, collection_name=coll_name, k=k_fetch,
            exclude_file=inputs.get("exclude_file"), filter_extensions=inputs.get("filter_extensions"),
            query_embedding=inputs.get("last_query_embedding") if inputs.get("last_query") == user_input else None,
        ...
        if USE_RERANKER and self.reranker and new_retrievals:
            ...
            new_retrievals = self.reranker.rerank(user_input, new_retrievals, top_k=RERANK_TOP_K)
        
        final_docs = self.budgeter.process_union(new_retrievals, previous_union, intent)
        full_context_docs, stable_str, new_str = self._prepare_context_strings(final_docs, previous_union, intent)
        ...
        active_llm = self.specialist_router.get_active_llm(specialist_model, self.llm, self.prompt)
        output_tokens = self.specialist_router.get_max_tokens(specialty, user_input)
        
        if output_tokens != MAX_TOKENS:
            active_llm = active_llm.bind(num_predict=output_tokens) if (specialist_model or self.model or "").startswith(OLLAMA_PREFIX) else active_llm.bind(max_tokens=output_tokens)
        ...
        if (turn_count > 0 and self._sentinel_failures["count"] < self.MAX_SENTINEL_FAILURES and 
            est_tokens >= SENTINEL_TOKEN_THRESHOLD and (turn_count - self._sentinel_cooldown["last_turn"]) >= SENTINEL_INTERVAL):
        ...
        if len("".join(full_answer)) > 50:
            sem_cache.upsert(user_input, "".join(full_answer), collection_scope=coll_name, pinned_content=pinned_content, model=self.model)
...
def build_rag_chain(db: Chroma, model: str | None = None):
    ...
    if is_cc:
        block_specs = [CORE_INSTRUCTIONS, "FULL SOURCE CONTEXT:\n{full_source_context}", "STABLE RAG CONTEXT:\n{stable_context}", "CONVERSATION STATE:\n{sentinel_state}", "NEW RAG DISCOVERIES:\n{new_context}"]
        system_blocks = [{"type": "text", "text": format_message_content(t, model, use_cache=(i < max_bp))} for i, t in enumerate(block_specs)]
        prompt = ChatPromptTemplate.from_messages([("system", system_blocks), MessagesPlaceholder("chat_history"), ("human", "{input}")])
    else:
        system_text = f"{CORE_INSTRUCTIONS}\n\nPINNED:\n{{full_source_context}}\n\nSTABLE:\n{{stable_context}}\n\nSTATE:\n{{sentinel_state}}\n\nNEW:\n{{new_context}}"
        prompt = ChatPromptTemplate.from_messages([("system", system_text), MessagesPlaceholder("chat_history"), ("human", "{input}")])

    from langchain_core.runnables import RunnableLambda
    return RunnableLambda(ContextCacheChain(db, model, llm, prompt, get_router(), get_reranker()))
