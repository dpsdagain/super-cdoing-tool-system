...
class ToolDispatcher:
    ...
    def execute_tool(
        self, name: str, args: Dict[str, Any], tool_id: str = "unknown"
    ) -> str:
        ...
        try:
            ...
            if isinstance(result, str):
                if result.startswith("[PLAN_UPDATED]"):
                    self.engine.state.current_plan = result.replace(
                        "[PLAN_UPDATED] ", ""
                    )
                elif result.startswith("[STATUS_UPDATED]"):
                    self.engine.state.current_status = result.replace(
                        "[STATUS_UPDATED] ", ""
                    )

            # SYSTEM 2: Result Budget Gate (Archiving)
            offloaded_result = (
                self.engine.context_manager.result_archive.offload_if_large(
                    name, str(result)
                )
            )
    ...
    def process_tool_calls(self, tool_calls, state, session_id):
        for tc in tool_calls:
            ...
            perm_decision = self.engine.permission_manager.check_permission(
                tool_name, tool_args
            )
            if perm_decision.behavior == "deny":
                ...
                if self.engine.state.permission_callback:
                    is_allowed = self.engine.state.permission_callback(
                        tool_name, tool_args
                    )
                    if not is_allowed:
                    ...
                    yield {
                        "type": "status",
                        "content": "Blocked: Permission callback missing.",
                    }
            ...
            if perm_decision.behavior == "allow":
                ...
                for hook in self.engine.state.hooks:
