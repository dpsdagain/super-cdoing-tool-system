...
class QueryEngine:
    ...
    def process_query(self, query: str, session_id: str = "default") -> tuple[str, List[Any]]:
    ...
    def process_query_stream(self, query: str, session_id: str = "default", messages: Optional[List[Any]] = None, max_turns: int = 15):
        ...
        while not state.is_terminal:
            ...
            try:
                ...
                if orphans:
                    logger.warning(f"Tombstone Action: Purging {len(orphans)} orphaned messages.")
                    for msg in orphans:
    ...
    def _execute_tick(self, state: QueryState, session_id: str):
        ...
        # 1. Decision Layer
        planner_decision = self.planner.call_planner(messages, state.turn_count, state.executed_actions)
        ...
        if state.executed_actions.count(action_sig) >= 2: action = "tool" 
        ...
        if action == "rag":
            ...
            try:
                ...
                for event in self.rag_chain.stream({"input": action_input, "chat_history": messages}):
                    if isinstance(event, dict) and "answer" in event: rag_result += event["answer"]
            ...
            try:
                ...
                if not response.tool_calls:
                    ...
                    yield from self.dispatcher.process_tool_calls(response.tool_calls, state, session_id)
            except Exception as e:
            ...
            if action == "tool" and not state.is_terminal:
                ...
                nudge = SystemMessage(content="If external action is required, you MUST call a tool now.")
                full_response = None
                for chunk in self.llm_with_tools.stream(state.messages + [nudge]):
                    if full_response is None: full_response = chunk
                ...
                if not full_response.tool_calls:
                    if state.turn_count > 1: action = "final"
                    else: return 

                yield from self.dispatcher.process_tool_calls(full_response.tool_calls, state, session_id)
