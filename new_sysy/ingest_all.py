...
def main():
    ...
    try:
        ...
        chunks = load_and_chunk_codebase(
            str(WORKSPACE_ROOT), on_progress=progress_callback
        )

        if not chunks:
            logger.warning("No files found matching your CODE_EXTENSIONS in config.py!")
            return
        ...
        logger.info(
            "Embedding chunks and saving to ChromaDB (this may take a minute)..."
        )
        ...
        print(
            "Done! You can now use the CLI to ask questions about your whole project."
        )
