...
def _analyze_query_intent(query: str):
    ...
    retrieval_anchors = re.findall(r'([a-zA-Z0-9_/.]+\.[a-z]{1,4}|[A-Z][a-z]+[A-Z][a-z]+|[a-z]+_[a-z_]+|[A-Z]{3,}_[A-Z0-9_]+)', query)
    ...
    is_aggregation_query = any(
        kw in query.lower() for kw in _AGGREGATION_KEYWORDS
    )
...
def hybrid_search(
    ...
    k_fetch = k * 3
    if query_embedding:
        vector_docs = db.similarity_search_by_vector(query_embedding, k=k_fetch, filter=_chroma_filter)
    else:
        vector_docs = db.similarity_search(search_query, k=k_fetch, filter=_chroma_filter)
    ...
    _rank_docs([d for d in new_retrievals if (d.metadata.get("source"), d.metadata.get("chunk_index", 0), d.metadata.get("content_hash", d.page_content[:64])) not in scores], weight=0.5)
